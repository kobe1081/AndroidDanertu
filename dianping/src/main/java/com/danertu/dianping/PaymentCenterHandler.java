...
//		pa.txt_submitOrder.setClickable(true);
        ...
        if (msg.what == WHAT_ORDER_SUCCESS) {
        ...
        } else if (msg.what == PaymentCenterActivity.RQF_PAY) {
        ...
        } else if (msg.what == WHAT_GETJLB_FAIL) {
            CommonTools.showShortToast(pa, "获取金萝卜失败");
        } else if (msg.what == WHAT_GETYUNFEI_SUCCESS) {

            pa.initView();

        } else if (msg.what == WHAT_ORDER_FAIL) {
            pa.hideLoadDialog();
        ...
        } else if (msg.what == WHAT_ORDER_ALREADY_SUBMIT) {
            pa.hideLoadDialog();
            if (pa.isIsStock()) {
//                CommonTools.showShortToast(pa, "订单已经提交过,请到订单中心查看");
        ...
        } else if (msg.what == WHAT_WECHAT_PAY) {
            //微信支付
            if (pa.isIsStock()) {
                if (msg.arg1 == 0) {
