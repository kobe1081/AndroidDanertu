...
public class CustomScrollView extends ScrollView {
    ...
    private boolean isCount = false;// 是否需要计算

    private boolean isMoving = false;// 是否需要移动.
    ...
    /***
     * 根据 XML 生成视图工作完成.该函数在生成视图的最后调用，在所有子视图添加完之后即使子类覆盖了onFinishInflate
    ...
    public void commOnTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                /**
                 * 排除出第一次移动计算，因为第一次无法得知y坐标在MotionEvent.ACTION_DOWN中获取不到，
                 * 因为此时是MyScrollView的touch事件传递到到了ListView的孩子item上面.从第二次计算.
                 * 然后我们也要进行初始化，就是第一次移动的时候让滑动距离归0. 之后记录准确了就正常执行.
                 */
