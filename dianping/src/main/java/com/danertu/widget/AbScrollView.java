...
public class AbScrollView extends ScrollView {
    ...
    /**
     * 根据XML生成视图工作完成，该函数在生成视图的时候调用，在还在视图添加完成之后， 即使子类覆盖了onFinishInflate
    ...
    private void commOnTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                 ...
                 * 因为此时是MyScrollView的Touch时间传递到了ListView的孩子item上面。所以从第二次开始计算
                 * 然后我们也要进行初始化，就是第一次移动的时让滑动距离归零，之后记录准确了就正常执行
                 */
                float preY = y;
                float nowY = ev.getY();
